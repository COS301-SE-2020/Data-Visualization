...
function main() {
    /**
     * This is the testing function, called at the end of the main() function.
     * It mostly exists due to the huge 'xmlData' and 'mock' definitions and is called
     * after they are defined.
     * No matter what, the corresponding functions being tested should work on this data.
     */
    function test() {
        let controller = graphSuggesterControllerMaker.getInstance();

        if (controller != null) {
            console.log ( 'Successfully constructed controller' );
        }
        else {
            console.log( 'Failed to construct controller' );
            console.log( 'Testing cannot proceed' );
            throw "Failed to construct graphSuggestionController";
        }

        console.log ( 'Parsing XML Data' );
        console.log ( '=======================================' );

        let parsed = controller.parseODataMetadata(xmlData);

        console.log ( 'Parsed successfully' );
        console.log ( '=======================================' );

        let suggestion = controller.getSuggestions ( JSON.stringify ( mock ) );
    ...
    //NOTE this is not an http request due to CORS errors
    //TODO If you can come up with a better solution than this(there likely is one out there) then do update this
    ...
    //this is test data obtained from https://services.odata.org/V2/Northwind/Northwind.svc/Products?format=json&$top=3
    //NOTE this is not an http request due to CORS errors
    //TODO If you can come up with a better solution than this(there likely is one out there) then do update this
    let mock = {
        "d" : [
            {
                ...
                "__metadata": {
                    "uri": "https://services.odata.org/V2/Northwind/Northwind.svc/Products(2)", "type": "NorthwindModel.Product"
                ...
                "__metadata": {
                    "uri": "https://services.odata.org/V2/Northwind/Northwind.svc/Products(3)", "type": "NorthwindModel.Product"
